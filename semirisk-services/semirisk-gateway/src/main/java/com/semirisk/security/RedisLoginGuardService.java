...
public class RedisLoginGuardService {
    ...
    public Optional<SemiRiskStore.LoginState> loginState(String username) {
        try {
            String lockValue = redisTemplate.opsForValue().get(lockKey(username));
            if (lockValue != null) {
                return Optional.of(new SemiRiskStore.LoginState(true, MAX_FAILURES, Instant.parse(lockValue)));
            }
            ...
            return Optional.of(new SemiRiskStore.LoginState(false, failures, null));
        } catch (Exception ex) {
            return Optional.empty();
        }
    }

    public Optional<SemiRiskStore.LoginState> recordFailure(String username) {
        try {
            String lockValue = redisTemplate.opsForValue().get(lockKey(username));
            if (lockValue != null) {
                return Optional.of(new SemiRiskStore.LoginState(true, MAX_FAILURES, Instant.parse(lockValue)));
            ...
            if (count >= MAX_FAILURES) {
                ...
                return Optional.of(new SemiRiskStore.LoginState(true, count, lockedUntil));
            }
            return Optional.of(new SemiRiskStore.LoginState(false, count, null));
        } catch (Exception ex) {
            return Optional.empty();
        }
    }
