...
public class SysLoginService {
    ...
    public void register(RegisterBody registerBody) {
        ...
        if (StringUtils.isBlank(registerBody.getEmail()) && StringUtils.isBlank(registerBody.getPhonenumber())) {
            throw new ServiceException("邮箱或手机号至少填写一个，用于账号找回");
        }
        // 校验用户类型是否存在
        ...
        remoteUserBo.setEmail(registerBody.getEmail());
        remoteUserBo.setPhonenumber(registerBody.getPhonenumber());
    ...
    public void forgotPassword(ForgotPasswordBody body) {
        ...
        if (captchaProperties.getEnabled()) {
            validateCaptcha(tenantId, username, body.getCode(), body.getUuid());
        }
        Boolean updated = remoteUserService.resetPasswordByUsername(tenantId, username, body.getContact(), BCrypt.hashpw(body.getNewPassword()));
        if (!updated) {
            throw new UserException("user.password.retry.limit.exceed", username);
        }
        recordLogininfor(tenantId, username, Constants.LOGIN_SUCCESS, "密码重置成功");
    }
