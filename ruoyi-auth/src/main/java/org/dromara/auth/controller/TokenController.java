...
import org.dromara.auth.form.ForgotPasswordBody;
 ...
 */
@Slf4j
@Tag(name = "Auth Token", description = "Login, registration, password recovery and token session endpoints")
...
public class TokenController {
    ...
    @Operation(summary = "Password login", description = "Returns a Sa-Token login token after captcha, tenant and client grant validation")
    @PostMapping("/login")
    ...
    @Operation(summary = "Register account", description = "Creates a system user in the database after captcha and registration switch validation")
    ...
    @ApiEncrypt
    @RateLimiter(time = 60, count = 5, limitType = LimitType.IP)
    @Operation(summary = "Recover password", description = "Resets password only after captcha, tenant and registered email or phone verification")
    @PostMapping("forgot-password")
    public R<Void> forgotPassword(@Valid @RequestBody ForgotPasswordBody body) {
        sysLoginService.checkTenant(body.getTenantId());
        sysLoginService.forgotPassword(body);
        return R.ok();
    }
    ...
    @Operation(summary = "Tenant list", description = "Returns tenants available on the login page")
