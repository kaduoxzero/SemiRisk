    ...
    document.querySelectorAll('button').forEach(btn => {
      if (btn.textContent.includes('连通性测试')) {
        btn.addEventListener('click', async () => {
          const data = await api('/api/system/models/ping', { method: 'POST', body: JSON.stringify({ model: btn.closest('.hud-card')?.innerText?.split('\n')[0] || 'model' }) });
          toast(`${data.model} 连通性测试成功，延时 ${data.latencyMs}ms`, 'success');
        });
      }
