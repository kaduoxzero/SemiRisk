...
public class RemoteUserServiceImpl implements RemoteUserService {
    ...
    public Boolean registerUserInfo(RemoteUserBo remoteUserBo) throws UserException, ServiceException {
        ...
        TenantHelper.dynamic(remoteUserBo.getTenantId(), () -> {
            if (StringUtils.isNotBlank(remoteUserBo.getEmail()) && userMapper.exists(new LambdaQueryWrapper<SysUser>()
                .eq(SysUser::getEmail, remoteUserBo.getEmail()))) {
                throw new ServiceException("注册邮箱已被使用");
            }
            if (StringUtils.isNotBlank(remoteUserBo.getPhonenumber()) && userMapper.exists(new LambdaQueryWrapper<SysUser>()
                .eq(SysUser::getPhonenumber, remoteUserBo.getPhonenumber()))) {
                throw new ServiceException("注册手机号已被使用");
            }
            return true;
        });
    ...
    @Override
    public Boolean resetPasswordByUsername(String tenantId, String username, String contact, String password) throws UserException, ServiceException {
        return TenantHelper.dynamic(tenantId, () -> {
            SysUserVo user = userMapper.selectVoOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getUserName, username));
            if (ObjectUtil.isNull(user)) {
                throw new UserException("user.not.exists", username);
            }
            boolean contactMatched = StringUtils.equalsIgnoreCase(contact, user.getEmail())
                || StringUtils.equals(contact, user.getPhonenumber());
            if (!contactMatched) {
                throw new ServiceException("registered email or phone does not match");
            }
            return userService.resetUserPwd(user.getUserId(), password) > 0;
        });
    }
